...
public class WindRotoDisplaySource extends NumericSingleLineDisplaySource {
    ...
    private record WindRotoValues(int generatedRpm, float generatedSu, float rawWindSpeed, float smoothedWindSpeed,
        ...
        private static WindRotoValues from(DisplayLinkContext context) {
            ...
            int rpm = windRoto.getGeneratedRpmForDisplay();
            float su = windRoto.getGeneratedSuForDisplay();
            float raw = windRoto.getRawWindSpeedForDisplay();
            float smooth = windRoto.getSmoothedWindSpeedForDisplay();
            if (!Float.isFinite(smooth)) {
                smooth = raw;
            }

            boolean outside = windRoto.isOutsideCachedForDisplay();
            int redstoneInput = clampRedstoneValue(windRoto.getExternalRedstoneInputForDisplay());
            boolean holdActive = windRoto.isStoppedByRedstoneForDisplay();

            int connectedServos = windRoto.getBoundServoCount();
            float averageServo = windRoto.getAverageBoundServoAngle();
            int sailLikeBlocks = windRoto.getSailLikeBlockCountForDisplay();
