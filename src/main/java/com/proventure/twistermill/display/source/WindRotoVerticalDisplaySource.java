...
public class WindRotoVerticalDisplaySource extends NumericSingleLineDisplaySource {
    ...
    private static final int MODE_SMOOTH_STONE_SLAB_NORTH_MARKER = 1;
    ...
    public List<MutableComponent> provideText(DisplayLinkContext context, DisplayTargetStats stats) {
        ...
        if (mode == MODE_SUMMARY_2) {
            ...
            lines.add(Component.literal("Smooth stone slab north marker: " + booleanText(values.markerPresent())));
    ...
    protected MutableComponent provideLine(DisplayLinkContext context, DisplayTargetStats stats) {
        ...
        return switch (getMode(context)) {
            ...
            case MODE_SMOOTH_STONE_SLAB_NORTH_MARKER -> Component.literal(booleanText(values.markerPresent()));
        ...
        builder.addSelectionScrollInput(0, 143,
                (selectionScrollInput, label) -> selectionScrollInput
                        .forOptions(List.of(
                                ...
                                Component.literal("Smooth stone slab north marker present"),
    ...
    private static boolean isTextMode(int mode) {
        return mode == MODE_VERTICAL_FACING
                || mode == MODE_SMOOTH_STONE_SLAB_NORTH_MARKER
    ...
    private record VerticalValues(boolean verticalFacing, boolean markerPresent, float yaw, int rpm,
        ...
        private static VerticalValues from(DisplayLinkContext context) {
            ...
            boolean markerPresent = windRotoVertical.isPlacementNorthValidForDisplay();
            float yaw = wrap360(windRotoVertical.getVerticalCurrentYawDegForDisplay());
            int rpm = windRotoVertical.getGeneratedRpmForDisplay();
            float targetYaw = wrap360(windRotoVertical.getVerticalTargetYawDegForDisplay());
            float worldWindAngle = wrap360(windRotoVertical.getLastWorldWindAngleDegForDisplay());
            float localTargetYaw = wrap360(windRotoVertical.getLastLocalTargetYawDegForDisplay());
            float yawSpeed = windRotoVertical.getVerticalYawVelocityDegPerTickForDisplay();
            boolean yawMoving = windRotoVertical.isVerticalYawMovingForDisplay();
            boolean parkMode = windRotoVertical.isVerticalParkedModeForDisplay();

            long cooldownUntil = windRotoVertical.getVerticalPulseCooldownUntilForDisplay();
            Level level = windRotoVertical.getLevel();
            long cooldownTicks = level == null ? 0L : Math.max(0L, cooldownUntil - level.getGameTime());

            return new VerticalValues(verticalFacing, markerPresent, yaw, rpm, targetYaw, worldWindAngle,
